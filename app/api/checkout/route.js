...
export async function POST(req, res) {
  ...
  try {
    ...
    if (body.card.number === withoutFundsCard) {
      ...
      // return
      return new Response(
        JSON.stringify(ERROR_CARD_WITHOUT_FUNDS, {
          status: 400,
        }),
      );
    }
    if (body.card.number === withoutAuthorizationCard) {
      ...
      // return
      return new Response(
        JSON.stringify(ERROR_CARD_WITHOUT_AUTHORIZATION, {
          status: 400,
        }),
      );
    }
    ...
    return new Response(
      JSON.stringify(ERROR_CARD_DATA_INCORRECT, {
        status: 400,
      }),
    );
